...
def main(arf_spon, arf_out, spon_entry_name='jrecord', out_entry_name='spon', verbose=False):
    '''copies entrys with spon_entry_name from arf_spon to arf_out, renaming with out_entry_name'''
    ...
    for entry in spon_entries:
        new_name = entry.name.strip('/')[-1].replace(spon_entry_name, out_entry_name)
        if verbose:
            print("copying, this may take a while...")
        out.copy(entry, new_name)
        if verbose:
            print('{} from {} copied to {} as {}'.format(entry.name, entry.file,
                                                         out.file, new_name))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('-s', '--arf-spon', help='arf file with spontaneous recordings',
                        required=True)
    parser.add_argument('-o', '--arf-out',
                        help='entrys from ARF_SPON will be copied here and renamed',
                        required=True)
